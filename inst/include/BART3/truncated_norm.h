...
double tnorm(double left, rn& gen)
{
    double rho, ppsl;
    int count = 1;
...
double tnorm(double left, double right, rn& gen)
{
    ...
    if (left >= 0) {
        ...
        return -1. * tnorm(-1.0 * right, -1.0 * left, gen);
    }
} 

double tnorm(double left, double mu, double sd, rn& gen)
{
    double newleft = (left - mu) / sd;
    return mu + tnorm(newleft, gen) * sd;
}

double tnorm(double left, double right, double mu, double sd, rn& gen)
{
    if (left==right) return left;
    ...
    double tdraw = tnorm(newleft, newright, gen);
...
double rtinvchi2(double scale, double trunc, rn& gen)
{
